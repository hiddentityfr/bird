...
const CompanyForm = (): JSX.Element => {
  ...
  const nextStep = () => {
    ...
    if (currentStep === 1) {
      if (
        !firstname ||
        !lastname ||
        !checkPassword(password as string, passwordConfirm as string)
      ) {
        return;
      }
    }
  ...
  const handleFirstname = (e: string) => {
    if (e != null) setFirstname(e);
  };
  const handleLastname = (e: string) => {
    if (e != null) setLastname(e);
  };
  const handlePassword = (e: string) => {
    if (e != null) setPassword(e);
  };
  const handlePasswordConfirm = (e: string) => {
    if (e != null) setPasswordConfirm(e);
  };
  const handleCompanyName = (e: string) => {
    if (e != null) setCompanyName(e);
  };
  const handleSiretNumber = (e: string) => {
    if (e != null) setSiretNumber(e);
  };
  const handleCompanySize = (e: number) => {
    if (e != null) setCompanySize(e);
  };

  const [sendCompanyRegister] = useMutation<CompanyResponse, CompanyVars>(
    api.company.mutations.createCompany,
    {
      onCompleted: () => {
    ...
    <Container row gap={0} align="stretch">
      ...
      <Container bg="#F2F2F5" gap={0} flex={1.8}>
        ...
        <Container>
          {step === 0 && (
            <Container>
              <Text variant="h2" color={theme.cvar('colorAccents2')}>
                Créer votre profil entreprise.
              </Text>
          ...
          {step === 1 && (
            <Container>
              ...
              <Text variant="h4" color={theme.cvar('colorAccents5')}>
                Créer votre profil entreprise est simple et ne dure que quelques
                minutes.
              </Text>
            </Container>
          )}
          {step === 2 && (
            <Container>
              ...
              <Text variant="h4" color={theme.cvar('colorAccents5')}>
                Veuillez renseigner les informations nécéssaires à la création
                de votre entreprise.
              </Text>
            </Container>
          )}
          {step === 3 && (
            <Container>
              ...
              <Text variant="h4" color={theme.cvar('colorAccents5')}>
                Vous pouvez rapidement inviter des membres dans votre
                entreprise.
              </Text>
            </Container>
          )}
          {step === 4 && (
            <Container flex={10} align="center">
              <Container flex={0}>
          ...
          {step === 0 && (
            <Container flex={0}>
              <Text
                weight="bold"
                variant="h5"
                color={theme.cvar('colorAccents4')}
              >
                Adresse mail
              </Text>
              <TextField
              ...
              <Container row flex={0}>
                <Container gap={0}>
                  ...
                  <TextField
                    ...
                    thickness="large"
                    onChange={(e) => {
                      handleCompanySize(parseInt(e, 10));
                    }}
                    type="number"
                    placeholder="1-10000"
                  />
